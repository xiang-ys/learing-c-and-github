...
int badData(SqList& L,int num);//输入错误数据
...
void DestroyList(SqList& L) {//销毁线性表
	for (int i = 0; i < L.length; i++) {//遍历
		if (L.elem[i] != NULL&&L.elem[i] != '0')L.elem[i] = NULL;
	}
	L.length = 0;
}

int badData(SqList& L,int num) {//输入错误数据处理
	if (num > L.length) {
		system("cls");
		cout << "你小子怎么能输入错误数据！！！！" << endl;
		Sleep(500);
		cout << "倒计时10秒后自动爆炸qwq" << endl;
		for (int i = 1; i <= 10; i++) {
			cout << i << "秒" << endl;
			Sleep(1000);
		}
